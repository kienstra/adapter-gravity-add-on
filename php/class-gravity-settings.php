...
class Gravity_Settings {
	...
	public function __construct() {
		add_filter( 'gform_form_settings', array( $this, 'aga_gform_add_label_option' ), 1, 2 );
		add_filter( 'gform_form_settings', array( $this, 'aga_gform_add_settings' ), 10, 2 );
	...
	function aga_gform_add_label_option( $settings, $form ) {
		...
		if ( $form_label_placement ) {
			$settings = $this->aga_add_label_placement_to_settings( $form_label_placement , $settings , $form );
		}
		return $settings;
	}

	function aga_add_label_placement_to_settings( $form_label_placement, $settings, $form ) {
		$new_option = $this->aga_get_option_for_in_placeholder( $form );
		$new_form_label_settings = $this->aga_get_new_form_label_settings( $new_option , $form_label_placement );
	...
	function aga_get_option_for_in_placeholder( $form ) {
		...
		$selected_attribute = $this->aga_get_placeholder_selected_attribute( $form );
